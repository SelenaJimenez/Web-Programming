...
def store():
    if request.method == "POST":
        title = request.form.get("title")         #Declarando las variables obtenidas desde el HTML

        if db.execute("Select * from books where title=:title",{"title":title}).rowcount == 1:
            return render_template("store.html", title=title)
        else:
            return render_template("review.html", message="Oops! Wrong")      
